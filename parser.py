...
def uncloop(lines, label, looplabels, text, labelnames, labelplaces, varnames, varvalues):
	...
	while lline:
		...
		#If remaindering two variables
			...
			if retvalue == '0':
				text = text + 'Exit function (return 0)\n'
			else:
				...
				text = text + 'Exit with return value: ' + retvalue
...
def logiparser(a, position, rootposition, rootvalue, text):
	...
	if sentence[0:3] == 'If ':
		...
		if rootvalue[-1] > 0:
			antecedent = text[0:rootvalue[-1]] + ' => ~' + '(' + anteline[anteline.find('If') + 3:-1] + ')'
		else:
			antecedent = '~' + '(' + anteline[anteline.find('If') + 3:-1] + ')'
...
for logictextpart in logictext:
	...
	print logiparser(logictextpart, 0, [], [], '')
