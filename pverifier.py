...
def recurs(sa, ia, reca, nbba, arga, refd):
	...
	if nbba[ia][1][0] == '1':
		if len(ns) > 2:
			v = recurs(sa + ns[1], int(ns[2]), reca + [ia], nbba, arga, refd)
		else:
			v = recurs(sa, int(ns[1]), reca + [ia], nbba, arga, refd)
		return v[0], v[1]
	elif nbba[ia][1][0] == '2':
		rs1 = recurs(vw + ns[1] + ');', int(ns[-2]), reca+[ia], nbba, arga, refd) 
		rs2 = recurs(nw + ns[1] + ');', int(ns[-1]), reca+[ia], nbba, arga, refd)
		return sa + rs1[0] + '\n' + sa + rs2[0] + '\n', rs1[1] + rs2[1]
	else:
		...
		for tk in bkeys:
			if tk in argarray:
				if d[tk][0] in ['+', '*', '%', '-', '.']:
					...
					t1v = 1 + (t1[0] == '2')
					t[1] = ', '.join(t1[:t1v] + [tk + ':=' + dtk[1] + dtk[0] + dtk[2]] + t1[t1v:])
